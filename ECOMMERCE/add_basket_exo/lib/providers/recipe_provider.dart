import 'package:add_basket_exo/models/recipe_model.dart';
import 'package:flutter/material.dart';

class RecipeProviders with ChangeNotifier {
  ...
  List<Recipe> get getRecipes => [..._getRecipes];

  List<Recipe> _favoriteRecipe = [];

  List<Recipe> get favoriteRecipe => [..._favoriteRecipe];

  void addFavorisCard(Recipe recipeId) {
    var index = _favoriteRecipe.indexOf(recipeId);
    if (index >= 0) {
      _favoriteRecipe.removeAt(index);
    } else {
      _favoriteRecipe.add(recipeId);
    }
    notifyListeners();
  }
}
